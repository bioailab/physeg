...
from helper_generator_v5 import generate_save_mito, get_config, save_physics_gt, save_physics_gt_rgb
...
def generate_mitochondria(batch, number, nlow, nhigh):
    np.random.seed(0)
    ...
    for t in range(size_t):
        ...
        save_noise_param='output/'+str(batch)+'/param/'+str(number)+'_'+str(t)+'.txt'
        ...
        if(t==0):
            ...
            #save_physics_gt_rgb(particlesArray_gt, batch, number, 130, 128, 4000) # todo: replace last 3 parameters
...
def parallel_run(batch,i,width,nlow,nhigh):
    generate_save_mito(batch,i, width, False)
    generate_mitochondria(batch,i,nlow,nhigh)
    
def generate_save_mito_in_batch(total_sample,batch):
    num_cores = 20#10#multiprocessing.cpu_count()
    config=get_config(batch)
    width=np.random.randint(int(config['wlow']),int(config['whigh']),total_sample+1)
    ...
    #with Pool(processes = num_cores) as p:
        #results = p.starmap_async(parallel_run, [(batch, i,width[i],nlow[i],nhigh[i]) for i in range(total_sample)])
    for i in range(total_sample):
        #parallel_run(batch, i,width[i],nlow[i],nhigh[i])
        pool.apply_async(parallel_run, args=(batch, i,width[i],nlow[i],nhigh[i]))
    pool.close()
...
if __name__ == "__main__":
    ...
    Path('datasave/'+str(batch)).mkdir(parents=True, exist_ok=True)
    from pathlib import Path
    ...
    generate_save_mito_in_batch(number_of_sample,batch)